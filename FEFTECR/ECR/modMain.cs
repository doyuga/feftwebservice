		...
		public static void CreateLoadSettings()
		{
			...
			try
			{
				...
				if (!File.Exists(text))
				{
					...
					dataRow["PortName"] = "COM1";
					dataRow["BaudRate"] = "9600";
					dataRow["Timeout"] = "10";
                ...
                mSearch = new ManagementObjectSearcher("root\\CIMV2", "SELECT * FROM Win32_PnPEntity WHERE ClassGuid=\"{4d36e978-e325-11ce-bfc1-08002be10318}\" and Description like'Vx%'");
                mReturn = mSearch.Get();
                ...
                if (mReturn.Count > 0)
                {
                    foreach (ManagementObject mObj in mReturn)
                    {
                            ...
                            string item = mObj["Caption"].ToString();
                            int indexOfCom = item.IndexOf("(COM");
                            modMain.Settings.PortName = item.Substring(indexOfCom + 1, item.Length - indexOfCom - 2);
